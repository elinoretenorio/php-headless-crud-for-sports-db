<?php

declare(strict_types=1);

namespace Sports\TeamAmericanFootballStats;

class TeamAmericanFootballStatsService implements ITeamAmericanFootballStatsService
{
    private ITeamAmericanFootballStatsRepository $repository;

    public function __construct(ITeamAmericanFootballStatsRepository $repository)
    {
        $this->repository = $repository;
    }

    public function insert(TeamAmericanFootballStatsModel $model): int
    {
        return $this->repository->insert($model->toDto());
    }

    public function update(TeamAmericanFootballStatsModel $model): int
    {
        return $this->repository->update($model->toDto());
    }

    public function get(int $id): ?TeamAmericanFootballStatsModel
    {
        $dto = $this->repository->get($id);
        if ($dto === null) {
            return null;
        }

        return new TeamAmericanFootballStatsModel($dto);
    }

    public function getAll(): array
    {
        $dtos = $this->repository->getAll();

        $result = [];
        /* @var TeamAmericanFootballStatsDto $dto */
        foreach ($dtos as $dto) {
            $result[] = new TeamAmericanFootballStatsModel($dto);
        }

        return $result;
    }

    public function delete(int $id): int
    {
        return $this->repository->delete($id);
    }

    public function createModel(array $row): ?TeamAmericanFootballStatsModel
    {
        if (empty($row)) {
            return null;
        }

        $dto = new TeamAmericanFootballStatsDto($row);

        return new TeamAmericanFootballStatsModel($dto);
    }
}